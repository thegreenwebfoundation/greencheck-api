...
class Validator
{
    ...
    public function validate($url, $ips)
    {
        ...
        if (false !== $ips['ipv4'] || false !== $ips['ipv6']) {
            return true;
        }

        $url = $this->getHostname($url);
        if ($this->validator->isValid($url)) {
            $this->errorMessages = ['invalidUrl' => 'No ip adress found for this url'];
        } else {
     ...
     *
     * @return bool True if valid ip adress, false otherwise
     */
    public function isUrlAValidIpAddress($ip)
    {
        ...
        if (method_exists($validator, 'validateValue')) {
            // sf 2.x validate
            $violations = $validator->validateValue($ip, new Ip(['version' => 'all']));
        } else {
            $violations = $validator->validate($ip, new Ip(['version' => 'all']));
        }
    ...
    public function isUrlAValidPublicIpAddress($ip)
    {
        ...
        if (method_exists($validator, 'validateValue')) {
            // Sf 2.x
            $violations = $validator->validateValue($ip, new Ip(['version' => 'all_public']));
        } else {
            // Sf 3
            $violations = $validator->validate($ip, new Ip(['version' => 'all_public']));
        }
    ...
    public function isValidIpAddressForType($ip, $type = '4')
    {
        ...
        if (method_exists($validator, 'validateValue')) {
            // Sf 2.x
            $violations = $validator->validateValue($ip, new Ip(['version' => $type.'_public']));
        } else {
            // Sf 3.x
            $violations = $validator->validate($ip, new Ip(['version' => $type.'_public']));
        }
        if (0 == count($violations)) {
            return true;
        }
